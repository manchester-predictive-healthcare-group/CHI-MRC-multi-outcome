...
DGM1 <- function(n, #number of patients to simulate
                 ...
                 beta12.x1, beta12.x2, beta12.x3, #covariate effects for transiion 12
                 beta13.x1, beta13.x2, beta13.x3, #covariate effects for transiion 13
                 beta23.x1, beta23.x2, beta23.x3, #covariate effects for transiion 23
                 ...
                 numsteps) #number of sampler steps in gems data generation process
{
  ...
  hf[[1, 2]] <- function(t, shape, scale, beta.x1, beta.x2, beta.x3) {
    exp(bl["x1"]*beta.x1 + bl["x2"]*beta.x2 + beta.x3*(bl["x3"])^2)*(shape/scale)*(t/scale)^(shape - 1)}
...
### Lets produce a calibration plot for CSH12
create_plot_csh <- function(data.mstate, data.raw, scenario){
  ...
  for (transition in 1:3){
    ...
    ### Create a dataset upon fit to fit the calibration model, where we will model time until outcome 1
    print(transition)
    data.csh <- subset(data.mstate, trans == transition)

    if (transition == 1){
      data.csh$lp <- data.raw$lp1
      data.csh$tp <- data.raw$tp12
    } else if (transition == 2){
      data.csh$lp <- data.raw$lp2
      data.csh$tp <- data.raw$tp13
    } else if (transition == 3){
      data.csh$lp <- data.raw$lp3[data.raw$id %in% data.csh$id]
      data.csh$tp <- data.raw$tp23[data.raw$id %in% data.csh$id]
    }

    ### Fit the cox model, using lp1 as the only predictors
    calib.model <- coxph(Surv(time, status) ~ lp, data = data.csh)

    ### Now generate predicted-pbserved values
    ### Extract the baseline hazard at 1825
    basehaz <- basehaz(calib.model, centered = FALSE)
    if (transition %in% c(1,2)){
      basehaz <- basehaz$hazard[min(which(basehaz$time >= 1825))]
    } else if (transition == 3){
      basehaz <- basehaz$hazard[min(which(basehaz$time >= 731))]
    }

    ### Calculate predicted-observed risks
    data.csh$pred.obs <- 1-exp(-basehaz*exp(data.csh$lp))

    ### Plot predo.obs1, vs tp12
    plots.list[[transition]] <- ggplot(aes(x = tp, y = pred.obs), data = data.csh) +
      geom_line() +
      geom_abline(slope = 1, intercept = 0, lty = "dotted") +
      ggtitle(paste("Transition ", transition, ", ", c(5,5,2)[transition], "-year risk", sep = "")) +xlab("Net risk") + ylab("Observed net risk")

  }

  ### Combine plots into single ggplot
  plots.comb <- ggpubr::ggarrange(plotlist = plots.list, nrow = 2, ncol = 2, common.legend = TRUE)

  ggsave(paste("plots/calibplot_CSH_scen", scenario, ".png", sep = ""), plots.comb, dpi = 300)
