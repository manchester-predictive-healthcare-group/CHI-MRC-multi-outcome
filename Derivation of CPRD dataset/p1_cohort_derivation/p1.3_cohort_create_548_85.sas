/* Create a cohort which requires 548 days prior follow up, and an index date < age 85 */
...
data person;
	...
	dt_age85 = round(dob + 85*365.25, 1);
	study_dtindex_A = max(regstartdate + 548, &studystartdate, dt_age65);
	format regstartdate best32.;
run;

proc print data = person(obs = 100); var person_id dt_age65 dt_age85 dob; run;
...
/* Note that given the way the min function works in sas, if death date will only be missing if both are missing */
data temp2;
	set temp;
...
/* Remove anyone for which index_date date is not prior to dtcens_comb, or prior to them being aged 85 */
data temp3;
	set temp2;
	if study_dtindex_A < dtcens_combdeath and study_dtindex_A < dt_age85 then output;
run;


/* I actually want to define two study_dtindex's */
/* The second one is at a random point between study_dtindex and min(dtcens_combdeath, age turned 85) */
...
data temp4;
	...
	*Multiply this by the length of the observation period for each individual;
	u_rand2 = u_rand*(min(dtcens_combdeath, dt_age85) - study_dtindex_A);
...
data datint.cohort_base_548_85;
	set temp4;
run;

proc contents data = datint.cohort_base_548_85;run;