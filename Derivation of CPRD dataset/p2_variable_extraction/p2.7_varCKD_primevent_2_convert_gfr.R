...
# Read in GFR data
mydata <- read.table("/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primevent_gfr_scores_A.txt", header=TRUE)
head(mydata)
...
## Export to csv file
write.csv(CKD_from_test_A,file="/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primevent_from_test_A.csv", row.names= FALSE)
...
# Read in GFR data
mydata <- read.table("/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primevent_gfr_scores_B.txt", header=TRUE)
head(mydata)
...
## Write to csv file
write.csv(CKD_from_test_B,file="/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primevent_from_test_B.csv", row.names= FALSE)

rm(list=setdiff(ls(),list("CKD_from_test_A", "CKD_from_test_B")))

## Save image
save.image("varCKD_from_test_primevent_image.RData")