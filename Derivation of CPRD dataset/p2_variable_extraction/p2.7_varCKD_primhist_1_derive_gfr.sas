...
data gfr_timeperiod (keep = person_id measurement_date gfr source source_num);
	...
	source = 'gfr';
	source_num = 0;
	if measurement_date <= study_dtindex_&cohort then output;
run;
...
proc export data=gfr_scores_A dbms = csv outfile="/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primhist_gfr_scores_A.csv" replace;run;
proc export data=gfr_scores_A dbms = tab outfile="/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primhist_gfr_scores_A.txt" replace;run;

proc export data=gfr_scores_B dbms = csv outfile="/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primhist_gfr_scores_B.csv" replace;run;
proc export data=gfr_scores_B dbms = tab outfile="/mnt/bmh01-rds/mrc-multi-outcome/code_cohort_extractions/Aurum_65plus/p2_variable_extraction/varCKD_primhist_gfr_scores_B.txt" replace;run;
