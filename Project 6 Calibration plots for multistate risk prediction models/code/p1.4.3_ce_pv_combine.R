...
for (state in 1:max.state){
  ...
  for (pctl in 1:n.pctls){
    for (set in 1:n.set){
      ### Load data
      load(paste("data/ce_pv_calc_set_N", n.devel, "_S", state, "_npctls", n.pctls, "_pctl", pctl, "_set", set, ".RData", sep = ""))
...
save.image(paste("data/ce_pv_combine_N", n.devel, "_npctls", n.pctls, ".RData", sep = ""))
