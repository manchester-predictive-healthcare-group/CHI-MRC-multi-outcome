...
#     num.groups <- 10
  ...
  ## Create object to store data and plots
  data.ggplot.true <- vector("list", max(data.mstate$to))
  ...
  for (i in 1:max(data.mstate$to)){
    ...
    ## Also create a dataset to plot the true calibration
    data.ggplot.true[[i]] <- data.frame("obs.true" = data.raw[paste("p.true", i, sep = "")], "pred" = p.est[paste("p.est", i, sep = "")])
    colnames(data.ggplot.true[[i]]) <- c("obs.true", "pred")
    
    ## Now create plot
    plots.ggplot[[i]] <- ggplot(data = data.ggplot[[i]], aes(x = pred, y = obs, color = "red")) +
      geom_point() +
      geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
      geom_line(data = data.ggplot.true[[i]], aes(x = pred, y = obs.true), color = "blue", linetype = "dotted") +
      xlab("Predicted risk") + ylab("Observed risk") + 
...
#   data.raw <- data.raw.reduc[1:20000, ]
  ...
  for (i in 1:5){
    ...
    data.pv$obs.true <- data.raw[, paste("p.true", i, sep = "")]
...
###
### Misc 4) Function to calculate IPC weights estimating the model from the data
###
calc.weights.stab <- function(data.raw, t.eval, max.weight = 10){
  ...
  ### First assign all individuals a weight of the probability of being uncesored at time t.eval
  data.raw$pcw.mspec <- data.weights.mspec$p.uncens[max(which(data.weights.mspec$time <= t.eval))]
  ### This quantity will also act as a the stabiliser in stabilised ipcws (marginal probability of being uncensored)
  data.raw$ipcw.numer <- data.weights.mspec$p.uncens[max(which(data.weights.mspec$time <= t.eval))]
  ...
  ### Finally create stabilised
  data.raw$ipcw.psec.stab <- data.raw$ipcw.numer*data.raw$ipcw.pspec
  
  ### Create output object
  output.object <- data.frame("ipcw.mspec" = data.raw$ipcw.mspec, "ipcw.pspec" = data.raw$ipcw.pspec, "ipcw.pspec.stab" = data.raw$ipcw.psec.stab)
...
###
### Misc 5) Function to calculate IPC weights using the DGM to assign them (i.e. perfect weights)
###
