###
### Assess moderate calibration for sensitivty analyses. Pseudo-value approach without grouping individuals by predicted risk,
### BLR-IPCW and MLR-IPCW without using IPCWs.
###
...
load(paste("data/sim/large_sample_combine_pv_sens_", scen, ".RData", sep = ""))
source("code/z_functions.R")

### Define output object
calib.moderate.pv.sens <- vector("list", 3)

### Calculate the calibration plots
for (i in 1:3){
  print(paste("calib pv", i , Sys.time(), sep = " "))
  calib.moderate.pv.sens[[i]] <- calib_msm(data.mstate = data.mstate,
                                      data.raw = data.raw,
                                      j = 1,
                                      s = 0,
                                      t = t.eval,
...
rm(list=setdiff(ls(), list("calib.moderate.pv.sens", "scen")))

### Save image
save.image(paste("data/sim/large_sample_analysis_moderate_sens_pv_", scen, ".RData", sep = ""))
print("IMAGE SAVED")
