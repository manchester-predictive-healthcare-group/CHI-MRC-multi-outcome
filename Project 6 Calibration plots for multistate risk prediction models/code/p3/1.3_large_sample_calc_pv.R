...
load(paste("data/sim/large_sample_prep_data_pv_", scen, "_npctls", n.pctls, ".RData", sep = ""))
...
pv.temp <- lapply(patids, func.calc.pv.aj.efficient,
                  data.mstate = subset(data.mstate, patid %in% data.sort.pctls[[state]][[pctl]]$patid),
