...
plots.comb.gg <- ggarrange(plotlist = plots.comb.list, nrow = 3, ncol = 5)
plots.comb.gg <- 
...
plots.comb.gg <- ggarrange(plotlist = plots.comb.list, nrow = 3, ncol = 5)
plots.comb.gg <- 
  annotate_figure(plots.comb.gg, left = "         Perfectly predicting                                               Over predicting                                               Under predicting        ")
...
for (i in 1:3){
  ...
  if (scen %in% c("M1C1", "M1C2", "M1C3")){
    ### Put into a list
    plots.comb.list <- c(calib.pv.list[[i]][["plots.list"]],
                         calib.blr.ipcw.list[[i]][["plots.pspec.list"]],
                         calib.mlr.ipcw.list[[i]][["plots.pspec.list"]])
    plots.comb.gg <- ggarrange(plotlist = plots.comb.list, nrow = 3, ncol = 5)
    plots.comb.gg <- 
      annotate_figure(plots.comb.gg, left = "         MLR-IPCW                                               BLR-IPCW                                               PV         ")
    ...
    plots.comb.list <- c(calib.aj[[i]][["plots.list"]],
                         calib.blr.ipcw.list[[i]][["plots.pspec.list"]],
                         calib.mlr.ipcw.list[[i]][["plots.pspec.list"]])
    plots.comb.gg <- ggarrange(plotlist = plots.comb.list, nrow = 3, ncol = 5)
    plots.comb.gg <- 
