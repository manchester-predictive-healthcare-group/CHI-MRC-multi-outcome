...
for (tpnum in 1:3){
  ...
  for (scen in c(do.call(paste0, expand.grid(c("M1"), c("C1", "C2", "C3"))))){
    ...
    for (state in 1:5){
      tables.est[[tpnum]][[state]][[scen]] <- round(c(calib.aj.list[[tpnum]][[state]],
                                                      calib.blr.ipcw.list[[tpnum]]$diff.pred.obs.pspec[[state]],
                                                      calib.mlr.ipcw.list[[tpnum]]$diff.pred.obs.pspec[[state]]) - calib.true[[tpnum]][state], 3)
      
      tables.est.se[[tpnum]][[state]][[scen]] <- round(c(0,
                                                         calib.blr.ipcw.boot.se.list[[tpnum]][["se"]][paste("diff.pred.obs.pspec", state, sep = "")],
                                                         calib.mlr.ipcw.boot.se.list[[tpnum]][["se"]][paste("diff.pred.obs.pspec", state, sep = "")]), 3)
    }
  }
}
...
print(gg.dw)
dev.off()
CairoTIFF(paste("figures/gg_large_sample_mean_N", n.cohort, "_npctls", n.pctls, ".tiff", sep = ""), 
         dpi = 300, width = 15, height = 10, unit = "in")
print(gg.dw)
dev.off()
CairoPDF(paste("figures/gg_large_sample_mean_N", n.cohort, "_npctls", n.pctls, ".pdf", sep = ""), 
         width = 15, height = 10)
print(gg.dw)
dev.off()
