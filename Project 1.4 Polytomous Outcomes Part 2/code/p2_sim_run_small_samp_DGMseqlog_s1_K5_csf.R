...
bounds <- calc.pred.quantiles.DGM.seqlog(K.sim, P.sim, coef.sim, beta0.sim)
...
for (i in 1:N.sim){
  ...
  data.valid <- generate.data.DGM.seqlog(K = K.sim, P = P.sim, coef = coef.sim, beta0 = beta0.sim, N = N.valid.sim)
...
save.image(paste("data/s", scenario.sim, "/sim_run_small_samp_DGMseqlog_scen" , scenario.sim, ".", seed.coef.sim, "_K", 
