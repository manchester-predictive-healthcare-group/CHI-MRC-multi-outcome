...
model {
    ...
    for (i in 1:stacked_N) {
      if (status[i] == 0){ //if censored
        if (outcome[i] == 1) {
          target += exponential_lccdf(times[i] | exp(omega[IDs[i]] + intercept_A + (X[i,]*betas_A)));
        }else if(outcome[i] == 2) {
          target += exponential_lccdf(times[i] | exp(omega[IDs[i]] + intercept_B + (X[i,]*betas_B)));  
        }
      }
      else if (status[i] == 1){ //if event/uncensored
        if (outcome[i] == 1) {
          target += exponential_lpdf(times[i] | exp(omega[IDs[i]] + intercept_A + (X[i,]*betas_A)));
        }else if(outcome[i] == 2) {
          target += exponential_lpdf(times[i] | exp(omega[IDs[i]] + intercept_B + (X[i,]*betas_B))); 
        }
      }
    }
}
