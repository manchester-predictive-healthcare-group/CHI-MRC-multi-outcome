...
model {
    ...
    target += exponential_lpdf(times_uncens_A | exp(log(omega[IDs_uncens_A]) + intercept_A + (X_uncens_A*betas_A)));
    target += exponential_lccdf(times_cens_A | exp(log(omega[IDs_cens_A]) + intercept_A + (X_cens_A*betas_A)));
    target += exponential_lpdf(times_uncens_B | exp(log(omega[IDs_uncens_B]) + intercept_B + (X_uncens_B*betas_B)));
    target += exponential_lccdf(times_cens_B | exp(log(omega[IDs_cens_B]) + intercept_B + (X_cens_B*betas_B)));
}
