...
generate.predicted.and.true.risks <- function(data.in){
  ...
  if ("msm" %in% anal.methods.sim){
    ...
    for (i in 1:nrow(data.out)){
      data.out[i, "predrisk.msm"] <- as.numeric(calc.risk.msm(fit.in = fit.msm, 
                                                              ...
                                                              x2.eval = data.out[i, "x2"])["risk.joint.est"])
    }
  }
  ...
  if ("c.frank" %in% anal.methods.sim){
    # Create empty vector
    data.out$predrisk.c.frank <- rep(0, nrow(data.out))
    # Run the loop
    for (i in 1:nrow(data.out)){
      data.out[i, "predrisk.c.frank"] <- as.numeric(calc.risk.copula(fit.in = fit.c.frank, 
                                                                     t.eval = t.eval.sim, 
                                                                     x1.eval = data.out[i, "x1"],
                                                                     x2.eval = data.out[i, "x2"])["risk.joint.est"])
    }
...
save.image("data/sim_function_run_simulation_2cont.RData")
