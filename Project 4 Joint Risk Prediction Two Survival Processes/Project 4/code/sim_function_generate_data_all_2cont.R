                        ...
                        numsteps) #number of sampler steps in gems data generation process
{
  ...
  hf[[1, 2]] <- function(t, shape, scale, beta.cont, beta.cont2) {
    exp(bl["x1"]*beta.cont + bl["x2"]*beta.cont2)*(shape/scale)*((t + sum(history))/scale)^(shape - 1)}
  
  hf[[1, 3]] <- function(t, shape, scale, beta.cont, beta.cont2) {
    exp(bl["x1"]*beta.cont + bl["x2"]*beta.cont2)*(shape/scale)*((t + sum(history))/scale)^(shape - 1)}
  
  hf[[2, 4]] <- function(t, shape, scale, beta.cont, beta.cont2) {
    exp(bl["x1"]*beta.cont + bl["x2"]*beta.cont2)*(shape/scale)*((t + sum(history))/scale)^(shape - 1)}
  
  hf[[3, 4]] <- function(t, shape, scale, beta.cont, beta.cont2) {
    exp(bl["x1"]*beta.cont + bl["x2"]*beta.cont2)*(shape/scale)*((t + sum(history))/scale)^(shape - 1)}
  ...
  par[[1, 2]] <- list(shape = shape12, scale = scale12, 
                      beta.cont = beta12.cont, beta.cont2 = beta12.cont2)
  par[[1, 3]] <- list(shape = shape13, scale = scale13, 
                      beta.cont = beta13.cont, beta.cont2 = beta13.cont2)
  par[[2, 4]] <- list(shape = shape24, scale = scale24, 
                      beta.cont = beta24.cont, beta.cont2 = beta24.cont2)
  par[[3, 4]] <- list(shape = shape34, scale = scale34, 
                      beta.cont = beta34.cont, beta.cont2 = beta34.cont2)
...
### Save image
##############
save.image("data/sim_function_generate_data_all_2cont.RData")
...
#                           beta12.cont = 0, beta12.cont2 = 0, #covariate effects for transiion 12
#                           beta13.cont = 0, beta13.cont2 = 0, #covariate effects for transiion 13
#                           beta24.cont = 0, beta24.cont2 = 0, #covariate effects for transiion 24
#                           beta34.cont = 0, beta34.cont2 = 0, #covariate effects for transiion 34
#                           x.in = x.baseline,
#                           numsteps = 50000)
# 
# 
