###
### Report event rates for scenario s1.1 under each DGM and sample size ###
###
...
for (DGM.in in c("msm", "c.clay", "c.gumb", "c.frank", "f.normal", "f.gamma")){
  ...
  if (DGM.in == "msm"){
    ...
    scale13.sim <- 123
    shape24.sim <- 1
    scale24.sim <- scale13.sim*0.77
    shape34.sim <- 1 
    scale34.sim <- scale12.sim*0.77
    
    ## Covariate effects for each transition
    beta12.cont.sim <- 0.25
    beta12.cont2.sim <- 0.6
    beta13.cont.sim <- 0.73
    beta13.cont2.sim <- 0.15
    beta24.cont.sim <- 0.73
    beta24.cont2.sim <- 0.15
    beta34.cont.sim <- 0.25
    beta34.cont2.sim <- 0.6
    ...
    if (DGM.in == "c.clay"){
      ...
      copula.sim <- "frank"
      rotate.cop.sim <- 0
      copula.param.sim <- 0.65
    }
  } else if(DGM.in %in% c("f.normal", "f.gamma")){
    if (DGM.in == "f.normal"){
      ## Baseline hazard parameters for marginal distributions
      baselineA.sim <- c(1,126)
      baselineB.sim <- c(1,131)
      ## Covariate effects
      COV_betaA.sim <- c(0.25, 0.6)
      COV_betaB.sim <- c(0.73, 0.15)
      ...
      ## Baseline hazard parameters for marginal distributions
      baselineA.sim <- c(1,112)
      baselineB.sim <- c(1,117)
      ## Covariate effects
      COV_betaA.sim <- c(0.27, 0.6)
      COV_betaB.sim <- c(0.73, 0.15)
      ## Frailty association parameter
      frail.var.sim <- 0.27
      ## Assigm distribution
...
save.image(paste("data/event_rates_s1.1.RData", sep = ""))
