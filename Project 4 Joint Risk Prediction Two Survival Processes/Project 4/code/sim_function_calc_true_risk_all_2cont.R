...
calc.true.risk.msm <- function(t.eval, x1.eval, x2.eval, #values of time, x1 and x2 to evaluate true risk at
                               ...
                               beta12.cont, beta12.cont2, #covariate effects for transiion 12
                               beta13.cont, beta13.cont2, #covariate effects for transiion 13
                               beta24.cont, beta24.cont2, #covariate effects for transiion 24
                               beta34.cont, beta34.cont2 #covariate effects for transiion 34
                               ){
  ...
  haz12 <- function(t.in, x1, x2){
    return(exp(beta12.cont*x1 + beta12.cont2*x2)*(shape12/scale12)*((t.in)/scale12)^(shape12 - 1))
  }
  
  haz13 <- function(t.in, x1, x2){
    return(exp(beta13.cont*x1 + beta13.cont2*x2)*(shape13/scale13)*((t.in)/scale13)^(shape13 - 1))
  }
  
  haz24 <- function(t.in, x1, x2){
    return(exp(beta24.cont*x1 + beta24.cont2*x2)*(shape24/scale24)*((t.in)/scale24)^(shape24 - 1))
  }
  
  haz34 <- function(t.in, x1, x2){
    return(exp(beta34.cont*x1 + beta34.cont2*x2)*(shape34/scale34)*((t.in)/scale34)^(shape34 - 1))
  }
  
  
  cumhaz12 <- function(u.in, t.in, x1, x2){
    return((exp(beta12.cont*x1 + beta12.cont2*x2)*(t.in/scale12)^(shape12)) - (exp(beta12.cont*x1 + beta12.cont2*x2)*(u.in/scale12)^(shape12)))
  }
  
  
  ## Cumulative hazard between u.in and t.in for each transition
  cumhaz13 <- function(u.in, t.in, x1, x2){
    return((exp(beta13.cont*x1 + beta13.cont2*x2)*(t.in/scale13)^(shape13)) - (exp(beta13.cont*x1 + beta13.cont2*x2)*(u.in/scale13)^(shape13)))
  }
  
  cumhaz24 <- function(u.in, t.in, x1, x2){
    return((exp(beta24.cont*x1 + beta24.cont2*x2)*(t.in/scale24)^(shape24)) - (exp(beta24.cont*x1 + beta24.cont2*x2)*(u.in/scale24)^(shape24)))
  }
  
  cumhaz34 <- function(u.in, t.in, x1, x2){
    return((exp(beta34.cont*x1 + beta34.cont2*x2)*(t.in/scale34)^(shape34)) - (exp(beta34.cont*x1 + beta34.cont2*x2)*(u.in/scale34)^(shape34)))
  }
...
### Save image
##############
save.image("data/sim_function_calc_true_risk_all_2cont.RData")
...
#                                     beta12.cont = 0.25, beta12.cont2 = 0.5,
#                                     beta13.cont = 0.5, beta13.cont2 = 0.5,
#                                     beta24.cont = 0.33, beta24.cont2 = 0.25,
#                                     beta34.cont = 0.25, beta34.cont2 = 0.25)
# true.risk.msm
# 
