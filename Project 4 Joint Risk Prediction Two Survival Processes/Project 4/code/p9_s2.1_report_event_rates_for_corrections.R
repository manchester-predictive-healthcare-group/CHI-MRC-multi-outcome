###
### Report event rates for scenario s2.1 under each DGM and sample size ###
###
...
for (DGM.in in c("msm", "c.clay", "c.gumb", "c.frank", "f.normal", "f.gamma")){
  ...
  if (DGM.in == "msm"){
    ...
    scale13.sim <- 38
    shape24.sim <- 1
    scale24.sim <- scale13.sim*0.61
    shape34.sim <- 1 
    scale34.sim <- scale12.sim*0.61
    
    ## Covariate effects for each transition
    beta12.cont.sim <- 0.22
    beta12.cont2.sim <- 0.8
    beta13.cont.sim <- 0.9
    beta13.cont2.sim <- 0.2
    beta24.cont.sim <- 0.9
    beta24.cont2.sim <- 0.2
    beta34.cont.sim <- 0.22
    beta34.cont2.sim <- 0.8
    ...
    if (DGM.in == "c.clay"){
      ...
      rotate.cop.sim <- 0
      ## Copula association parameter
      copula.param.sim <- 1.52
    }
  } else if(DGM.in %in% c("f.normal", "f.gamma")){
    if (DGM.in == "f.normal"){
      ## Baseline parameters
      baselineA.sim <- c(1,39.5)
      baselineB.sim <- c(1,39.5)
      ## Covariate effects
      COV_betaA.sim <- c(0.35, 0.85)
      COV_betaB.sim <- c(0.9, 0.3)
      ...
      ## Baseline hazard parameters for marginal distributions
      baselineA.sim <- c(1,31)
      baselineB.sim <- c(1,31)
      ## Covariate effects
      COV_betaA.sim <- c(0.35, 0.85)
      COV_betaB.sim <- c(0.9, 0.3)
      ## Frailty association parameter
...
save.image(paste("data/event_rates_s2.1.RData", sep = ""))
