...
model {
    ...
    omega ~ gamma(frail_param, frail_param);
    frail_param ~ gamma(0.001,0.001);
    
    for (i in 1:stacked_N) {
      if (status[i] == 1){ //if event/uncensored
        if (outcome[i] == 1) {
          target += weibull_lpdf(times[i] | shape_A, omega[IDs[i]]*exp(intercept_A + (X[i,]*betas_A)));
        }else if(outcome[i] == 2) {
          target += weibull_lpdf(times[i] | shape_B, omega[IDs[i]]*exp(intercept_B + (X[i,]*betas_B))); 
        }
      } else if (status[i] == 0){ //if censored
        if (outcome[i] == 1) {
          target += weibull_lccdf(times[i] | shape_A, omega[IDs[i]]*exp(intercept_A + (X[i,]*betas_A)));
        }else if(outcome[i] == 2) {
          target += weibull_lccdf(times[i] | shape_B, omega[IDs[i]]*exp(intercept_B + (X[i,]*betas_B)));  
        }
      }
    }
}
