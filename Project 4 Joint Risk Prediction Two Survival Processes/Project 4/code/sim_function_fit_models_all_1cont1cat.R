...
pat <- fit.in[["msm.data"]][min(which(fit.in[["msm.data"]]$x2 == x2.eval)),]

# Assign x1 to what we want it to be
pat$x1 <- x1.eval
...
#   baseline.dist <- "exp"
    ...
    stan_data <- list(N = length(unique(data.in$id)),
                         ...
                         X_cens_A = data.in.cens.A %>% dplyr::select(x1, x2) %>% mutate(x2 = as.numeric(x2 == 1)) %>% data.matrix(),
                         X_uncens_A = data.in.uncens.A %>% dplyr::select(x1, x2) %>% mutate(x2 = as.numeric(x2 == 1)) %>% data.matrix(),
                         X_cens_B = data.in.cens.B %>% dplyr::select(x1, x2) %>% mutate(x2 = as.numeric(x2 == 1)) %>% data.matrix(),
                         X_uncens_B = data.in.uncens.B %>% dplyr::select(x1, x2) %>% mutate(x2 = as.numeric(x2 == 1)) %>% data.matrix(),
...
save.image("data/sim_function_fit_models_all_1cont1cat.RData")
# 
# # 
