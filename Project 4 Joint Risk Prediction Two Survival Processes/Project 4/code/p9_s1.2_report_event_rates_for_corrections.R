###
### Report event rates for scenario s1.2 under each DGM and sample size ###
###
...
for (DGM.in in c("msm", "c.clay", "c.gumb", "c.frank", "f.normal", "f.gamma")){
  ...
  if (DGM.in == "msm"){
    ...
    scale13.sim <- 126
    shape24.sim <- 1
    scale24.sim <- scale13.sim*0.58
    shape34.sim <- 1 
    scale34.sim <- scale12.sim*0.58
    ...
    if (DGM.in == "c.clay"){
      ...
      rotate.cop.sim <- 0
      ## Copula association parameter
      copula.param.sim <- 1.37
    }
  } else if(DGM.in %in% c("f.normal", "f.gamma")){
    if (DGM.in == "f.normal"){
      ## Baseline hazard parameters for marginal distributions
      baselineA.sim <- c(1,144)
      baselineB.sim <- c(1,149)
      ## Covariate effects
      COV_betaA.sim <- c(0.25, 0.6)
      COV_betaB.sim <- c(0.73, 0.15)
      ...
      ## Baseline hazard parameters for marginal distributions
      baselineA.sim <- c(1,107)
      baselineB.sim <- c(1,112)
      ## Covariate effects
      COV_betaA.sim <- c(0.25, 0.6)
      COV_betaB.sim <- c(0.73, 0.15)
      ## Frailty association parameter
...
save.image(paste("data/event_rates_s1.2.RData", sep = ""))
