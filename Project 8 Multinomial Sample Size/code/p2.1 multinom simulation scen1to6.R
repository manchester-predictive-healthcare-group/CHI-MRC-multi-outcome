...
scenario1.res<-(foreach(input=c(250,500,1000,541,576), .combine=list, .multicombine=TRUE)
...
warnings()
Sys.time()
save.image("R_out/multinom simulation scen1to6 500000val.RData")
...
cl <- makeCluster(6)
registerDoParallel(6)
scenario2.res<-(foreach(input=c(250,500,1000,569,628), .combine=list, .multicombine=TRUE)
                %dopar%{run.multinom.sim(-0, .5, -0.25, -0.125, 0.25, 0.375,
                                         -0.75, 0.375, -0.5, -0.25, -0.375, 0.125, 
...
cl <- makeCluster(6)
registerDoParallel(6)
scenario3.res<-(foreach(input=c(250,500,1000,566,582), .combine=list, .multicombine=TRUE)
...
scenario4.res<-(foreach(input=c(250,500,1000,648,901), .combine=list, .multicombine=TRUE)
...
cl <- makeCluster(6)
registerDoParallel(6)
scenario5.res<-(foreach(input=c(250,500,1000,706,1458), .combine=list, .multicombine=TRUE)
...
cl <- makeCluster(6)
registerDoParallel(6)
scenario6.res<-(foreach(input=c(250,500,1000,1558,1616), .combine=list, .multicombine=TRUE)
                %dopar%{run.multinom.sim(-2.9, .5, -0.25, -0.125, 0.25, 0.375,
                                         -2.9, 0.375, -0.5, -0.25, -0.375, 0.125, 
