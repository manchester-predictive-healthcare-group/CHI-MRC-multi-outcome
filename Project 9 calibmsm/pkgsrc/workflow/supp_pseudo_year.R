...
for (state in 1:6){
  ...
  ###
  ### Create n.groups datasets, grouped by predicted risk
  ###

  ### Create a list of length n.groups
  data.sort.groups[[state]] <- vector("list", 3)
  for (group in 1:3){
    data.sort.groups[[state]][[group]] <- data.raw.list[[group]]
  }
  ...
  for (group in 1:3){
    print(paste("Calc obs.aj = ", state, "group = ", group, Sys.time()))
    obs.aj.groups[[state]][[group]] <- calc.calib.aj.ce(data.mstate = base::subset(data.mstate, id %in% data.sort.groups[[state]][[group]]$id),
...
for (state in 1:6){
  ...
  for(group in 1:3){
    ...
    ids.vec <- data.sort.groups[[state]][[group]]$id
    ### Calculate pseudo-values and store
    pv.temp <- lapply(ids.vec, func.calc.pv.aj.ce,
                      data.mstate = subset(data.mstate, id %in% data.sort.groups[[state]][[group]]$id),
                      obs.aj = obs.aj.groups[[state]][[group]],
                      tmat = attributes(data.mstate)$trans,
                      n.cohort = nrow(data.sort.groups[[state]][[group]]),
                      t.eval = t.eval)
    ...
    pv.out.groups[[state]][[group]] <- data.frame(cbind(ids.vec, pv.temp))
    colnames(pv.out.groups[[state]][[group]]) <- c("id", paste("pv.", state, sep = ""))
...
Cairo::CairoPNG(paste("workflow/figures/supp_pseudo_j1s0_pseudo_year.png", sep = ""),
